...
<body>
    <div id="wrap">
		...
		if(isset($_POST['submit']))
		{
			$op=$_POST['oldpass'];
			$np=$_POST['newpass'];
			$sql="select `password` from login where username='$user' and `password`='$op'";
			$res=mysqli_query($con,$sql);
			$rc=mysqli_num_rows($res);
			if($rc==1)
			{
				$sql="update login set `password`='$np' where username='$user'";
			$res=mysqli_query($con,$sql);
				echo "<script type='text/javascript'>alert('updated successfully'); location='index.php';</script>";
		}
		else
		{
			echo "<script type='text/javascript'>alert('wrong password'); location='logout.php';</script>";
		}
		}
		?>